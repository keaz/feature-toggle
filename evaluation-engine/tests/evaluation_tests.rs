use evaluation_engine::{Feature, FeatureEvaluationContext, FeatureStage, StageContext, StageCriterion, Context};

fn mk_ctx(feature: &str, env: &str, pairs: &[(&str, &str)]) -> FeatureEvaluationContext {
    FeatureEvaluationContext {
        feature: feature.to_string(),
        environment_id: env.to_string(),
        context: pairs.iter().map(|(k, v)| Context { key: (*k).into(), value: (*v).into() }).collect(),
    }
}

fn stage(env: &str, enabled: bool, bucketing: Option<&str>, criterias: Vec<StageCriterion>) -> FeatureStage {
    FeatureStage {
        environment_id: env.to_string(),
        enabled,
...
fn criterion(context_key: &str, allowed: &[&str], pct: i32) -> StageCriterion {
    StageCriterion {
        ...
        context: StageContext { key: context_key.to_string(), entries: allowed.iter().map(|s| (*s).into()).collect() },
...
fn evaluate_returns_false_when_feature_disabled() {
    ...
    let feature = Feature { enabled: false, dependencies: vec![], stages: vec![] };
...
fn evaluate_requires_matching_environment_stage() {
    ...
    let feature = Feature { enabled: true, dependencies: vec![], stages: vec![stg] };
    assert!(!evaluation_engine::evaluate(ctx, feature));
}
...
fn evaluate_fails_without_bucketing_identity_when_criteria_present() {
    ...
    let stg = stage("env-a", true, None, vec![criterion("country", &["US"], 100)]);
    let feature = Feature { enabled: true, dependencies: vec![], stages: vec![stg] };
    assert!(!evaluation_engine::evaluate(ctx, feature));
}
...
fn evaluate_respects_custom_bucketing_key() {
    ...
    let stg = stage("env-a", true, Some("userId"), vec![criterion("country", &["US"], 100)]);
    let feature = Feature { enabled: true, dependencies: vec![], stages: vec![stg] };
    assert!(evaluation_engine::evaluate(ctx, feature));
}
...
fn evaluate_respects_rollout_percentage_thresholds() {
    ...
    let f0 = Feature { enabled: true, dependencies: vec![], stages: vec![stg(0)] };
    ...
    let f100 = Feature { enabled: true, dependencies: vec![], stages: vec![stg(100)] };
...
fn evaluate_requires_matching_context_value() {
    ...
    let stg = stage("env-a", true, None, vec![criterion("country", &["US", "CA"], 100)]);
    let feature = Feature { enabled: true, dependencies: vec![], stages: vec![stg] };
    assert!(!evaluation_engine::evaluate(ctx, feature));
}
...
fn evaluate_all_dependencies_must_pass() {
    ...
    let dep1 = Feature { enabled: true, dependencies: vec![], stages: vec![stage("env", true, None, vec![])] };
    let dep2 = Feature { enabled: true, dependencies: vec![], stages: vec![stage("env", false, None, vec![])] };

    let root = Feature { enabled: true, dependencies: vec![dep1, dep2], stages: vec![stage("env", true, None, vec![])] };
...
fn evaluate_nested_dependencies_true() {
    ...
    let leaf = Feature { enabled: true, dependencies: vec![], stages: vec![stage("env", true, None, vec![])] };
    let mid = Feature { enabled: true, dependencies: vec![leaf], stages: vec![stage("env", true, None, vec![])] };
    let root = Feature { enabled: true, dependencies: vec![mid], stages: vec![stage("env", true, None, vec![])] };
    assert!(evaluation_engine::evaluate(ctx, root));
}
